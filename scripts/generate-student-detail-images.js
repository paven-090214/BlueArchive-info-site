...
const DETAIL_DIR = path.join(PROJECT_ROOT, "images", "students", "detail");
...
async function main() {
  ...
  const userDataDir = fs.mkdtempSync(path.join(os.tmpdir(), "ba-detail-image-chrome-"));
  ...
  try {
    ...
    for (const portrait of portraits) {
      const outputPath = path.join(DETAIL_DIR, portrait.name);
      ...
      try {
        ...
        const generated = await generateDetailImage(ws, base64);
    ...
    console.log(`Output: ${path.relative(PROJECT_ROOT, DETAIL_DIR)}`);
  } finally {
    chrome.kill();
    await wait(500);
...
async function fetchExpectedStudentIds() {
  ...
  console.warn(`[detail image generation] expected student list unavailable: ${errors.join(" / ")}`);
  return [];
}
...
function printSummary({
  ...
  console.log("[detail image generation]");
...
async function generateDetailImage(ws, sourceBase64) {
  const expression = `
    (async () => {
      const sourceBase64 = ${JSON.stringify(sourceBase64)};
      const detailWidth = ${DETAIL_WIDTH};
      const detailHeight = ${DETAIL_HEIGHT};
      ...
      const availableHeight = detailHeight - topPadding - bottomPadding;
      const heightScale = availableHeight / box.height;
      const widthScale = detailWidth / box.width;
      const scale = Math.max(heightScale, Math.min(widthScale, heightScale));
      const centerX = box.left + box.width / 2;
      const drawWidth = sourceCanvas.width * scale;
      const drawHeight = sourceCanvas.height * scale;
      const drawX = detailWidth / 2 - centerX * scale;
      const drawY = detailHeight - bottomPadding - box.bottom * scale;
      ...
      detailContext.drawImage(sourceCanvas, drawX, drawY, drawWidth, drawHeight);

      return detailCanvas.toDataURL("image/webp", webpQuality).split(",")[1];
